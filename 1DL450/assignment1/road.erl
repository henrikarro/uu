...
days_alt(Length, [], Goal, DaysSoFar, IntervalsSoFar) ->
    case longest_interval(invert_intervals(IntervalsSoFar, 0, Length)) =< Goal of
        true -> DaysSoFar;
        false -> -1
    end;
days_alt(Length, [Interval|Intervals], Goal, DaysSoFar, IntervalsSoFar) ->
    case longest_interval(invert_intervals(IntervalsSoFar, 0, Length)) =< Goal of
        true -> DaysSoFar;
        false -> days_alt(Length, Intervals, Goal, DaysSoFar + 1, add_interval(Interval, IntervalsSoFar))
    end.
